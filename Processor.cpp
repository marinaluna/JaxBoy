...
Processor::Processor()
{
    ...
    // Start with interrupts disabled since they can't
    // be used in bootrom anyway
...
int Processor::Tick()
{
    ...
    if(reg_PC == 0x100)
    {
        // After the bootrom has completed
        GameBoy::GetGameBoy()->ReplaceInterruptVectors();
    }
...
int Processor::ExecuteAt(uint16_t address) // Decodes and executes instruction
{
    ...
    {
        SETFLAGS(
            (reg_AF.High() |= GameBoy::Read8(reg_HL)) == 0? 1 : 0,
            0, 0, 0);
        SETLENGTHCYCLES(1, 8);
        break;
    }
    ...
    case 0xC3: // JP imm16
    {
        reg_PC = uint16_t(GameBoy::Read16(address + 1));
    ...
    {
        reg_SP -= 2;
        GameBoy::Write16(reg_SP, reg_AF);
        SETLENGTHCYCLES(1, 16);
        break;
    }
    case 0xFA: // LD A, (imm16)
    {
        reg_AF.High() = GameBoy::Read16(address + 1);
        SETLENGTHCYCLES(3, 16);
        break;
    }
    case 0xFB: // EI
    ...
    default:
        GameBoy::SystemError("Unimplemented OPCODE  %s  at address  %04Xh!\n", OPCODE_LOOKUP[opcode], address);
    }
...
void Processor::ExecuteCBOpcode(uint16_t address, int& cycles_this_tick, int& instruction_length)
{
    ...
    case 0x87: // RES 0, A
    {
        SETBIT(BIT0_MASK, reg_AF.High(), 0);
        SETLENGTHCYCLES(2, 8);
        break;
    }
    default:
